...
export async function POST(request: Request): Promise<Response> {
  ...
  const body = (await request.json().catch(() => null)) as { css?: unknown; source?: unknown; versionName?: unknown } | null;
  ...
  if (!versionName) return Response.json({ success: false, errors: [{ type: "validation", message: "请填写版本名称" }] }, { status: 400 });
  ...
  const saved = await saveUserTheme({ userId, css, versionName });
